...
Генетический алгоритм, в свою очередь, состоит из трех основных компонентов: генетическое представление, функция приспособленности и генетические операторы. Генетическое представление – это способ представления возможных решений в виде генотипа (например, набора битов), который может быть интерпретирован как фенотип (например, численное значение). Функция приспособленности – это мера качества каждого возможного решения, которая определяет, насколько хорошо это решение решает задачу оптимизации.
Генетические операторы – это операции, которые применяются к генотипам (например, скрещивание и мутация), чтобы создавать новые генотипы, которые могут быть протестированы с помощью функции приспособленности.
В генетическом алгоритме начальная популяция генотипов создается случайным образом, а затем путем применения генетических операторов создаются новые популяции. Процесс повторяется несколько раз до тех пор, пока не будет найдено оптимальное решение или пока не будет достигнут лимит итераций (рис. 13).
